...
st.markdown(f"""
    <div class="report-header">
        ...
        <p style="margin:5px 0 0 0; color:#E6002D; font-weight:bold;">FF 인바운드 실시간 전략 분석 리포트 (v11.0)</p>
    </div>
...
# 6. 10대 선사 통합 데이터 엔진 (상태 세분화 및 주요사항 통합)
def get_intel_data(pol_val):
    route_oman = f"🌐 **[Oman Bypass]** {pol_val} → 살랄라(Salalah) 하역 → **보세 운송(Bonded)** → Al Batha 국경 → {pod}"
    route_cape = f"🌐 **[Cape Detour]** {pol_val} → **희망봉(Cape) 우회** → 지브롤터 → 수에즈(N) → **제다(Jeddah) 하역** → {pod}"
