...
# ==========================================
st.set_page_config(page_title="Hormuz Crisis Monitor v158", layout="wide")
...
def run_integrated_report(api_key, is_ko):
    try:
        ...
        # 💡 [핵심 지시 반영] 1. 운임 제외 FM 선언일 표 2. 항구별 항만청 발표 3. 실시간 속보 링크
        queries = [
            f"""[지시 1: 선사 FM 현황 표] {today} 기준 아래 팩트 데이터를 사용하여 표를 작성하세요. 
            운임 수치는 절대 기재하지 마십시오. 선사명은 <font color='red'><b>선사명</b></font> 처리.
            (항목: 선사명, 공식 양하항, FM 선언일, FM 상태)
            - MSC: Salalah, 2026-03-02 / Maersk: Salalah, 2026-03-01
            - COSCO: Khalifa, 2026-03-03 / CMA CGM: Jebel Ali, 2026-03-02
            - Hapag-Lloyd: Salalah, 2026-03-01 / ONE: Salalah, 2026-03-03
            - Evergreen: Salalah, 2026-03-04 / HMM: Sohar, 2026-03-02
            - Yang Ming: Sohar, 2026-03-02 / ZIM: Jebel Ali, 2026-02-28
            (언어: {'한국어' if is_ko else 'English'})""",
            
            f"""[지시 2: 항만별 독립 리포트] {today} 기준 Salalah, Sohar, Jebel Ali 각 항만청의 공식 발표 내용을 섹션별로 분리하여 리포트하세요. 
            호르무즈 봉쇄가 항구 운영에 미치는 물리적 영향(피더 중단, 야드 디깅 지연 등) 팩트만 기재하세요. (언어: {'한국어' if is_ko else 'English'})""",
            
            f"""[지시 3: 실시간 전황 속보] {today} 기준 이란-이스라엘 전쟁 속보를 [🔴 이란/아랍편]과 [🔵 미국/서방편]으로 나누어 리포트하세요. 
            반드시 실제 기사 제목, 요약 번역, 클릭 가능한 URL 링크를 포함하세요. 예측이나 가상 문구는 절대 금지합니다. (언어: {'한국어' if is_ko else 'English'})"""
        ]
...
st.markdown(f'<div class="report-header"><h1 style="margin:0;">{title} (v158.0)</h1></div>', unsafe_allow_html=True)
