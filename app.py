...
current_time = now.strftime("%Y-%m-%d %H:%M:%S (KSA)")
...
st.markdown(f"""
    ...
    <div class="update-box"><strong>{ '실시간 시황 및 프로세스 검증 시점' if is_ko else 'Real-time Intel Verified at' }:</strong> {current_time}</div>
""", unsafe_allow_html=True)

# 5. 다국어 데이터 엔진 (상태/라우트/주요사항 모두 영어 지원)
def get_intel_data():
    if is_ko:
        ...
        return [
            ["Maersk", "제다:🟢(우회)\n담맘:🔴(중단)\n**타항:Khor Fakkan**\n**종합:부킹 중단**", route_uae.replace(" / Fujairah", ""), "📢 **공지:** 호르무즈 해협 진입 불가로 코르파칸 우회 노선 집중\n💰 **비용:** UAE-리야드 약 $1,900~$2,300\n🔗 **보세:** 가능"],
            ["MSC", "제다:🟡(협의)\n담맘:🔴(중단)\n**타항:Salalah**\n**종합:부킹 제한**", route_oman, "📢 **기사:** 살랄라 하역 후 Rub Al Khali 직통 노선 이용 권고\n💰 **비용:** 살랄라-리야드 약 $2,300~$2,600\n🔗 **보세:** 가능"],
            ["CMA CGM", "제다:🟢(우회)\n담맘:🔴(중단)\n**타항:Fujairah**\n**종합:희망봉 우회**", route_uae.replace("Khor Fakkan / ", ""), "📢 **기사:** UAE 동부 푸자이라 하역 후 Al Batha 국경 연계 서비스 가동\n💰 **비용:** UAE-리야드 약 $1,800~$2,200\n🔗 **보세:** 가능"],
            ["Hapag-Lloyd", "제다:🟢(우회)\n담맘:🔴(중단)\n**타항:Khor Fakkan**\n**종합:제다 우회**", route_uae.replace(" / Fujairah", ""), "📢 **공지:** 코르파칸 및 제다 하역 후 육로 전환 솔루션 제공 중\n💰 **비용:** UAE-리야드 약 $2,000~$2,400\n🔗 **보세:** 가능"],
            ["HMM", "제다:🟡(대기)\n담맘:🔴(중단)\n**타항:검토중**\n**종합:부킹 제한**", "Suspended", "📢 **공지:** 국적선사 안전 지침에 따라 걸프향 신규 예약 전면 중단\n💰 **비용:** 확인 요망\n🔗 **보세:** 협의 필요"],
            ["ONE", "제다:🟡(대기)\n담맘:🔴(중단)\n**타항:Khor Fakkan**\n**종합:부킹 제한**", route_uae.replace(" / Fujairah", ""), "📢 **기사:** UAE 동부항 임시 양하 후 사우디향 육로 셔틀 서비스 검토 중\n💰 **비용:** UAE-리야드 약 $2,000~$2,300\n🔗 **보세:** 가능"],
            ["Evergreen", "제다:🟢(우회)\n담맘:🔴(중단)\n**타항:없음**\n**종합:희망봉 우회**", route_cape, "📢 **공지:** 희망봉 우회 채택으로 인한 리드타임 25일 이상 추가 지연\n💰 **비용:** 제다-리야드 약 $1,400~$1,700\n🔗 **보세:** 가능"]
        ]
    else:
        # 영어 데이터 (English Version)
        ...
        route_cape = "🌐 **[Cape Detour]** FE → **Cape Detour** → Suez(N) → **Jeddah Port** → Inland → Riyadh"
        return [
            ["Maersk", "Jeddah:🟢(Detour)\nDammam:🔴(Stop)\n**Port:Khor Fakkan**\n**Status:Suspended**", route_uae.replace(" / Fujairah", ""), "📢 **Notice:** All Gulf bookings suspended; Khor Fakkan detour active\n💰 **Cost:** UAE-Riyadh approx $1,900~$2,300\n🔗 **Bonded:** YES"],
            ...
            ["CMA CGM", "Jeddah:🟢(Detour)\nDammam:🔴(Stop)\n**Port:Fujairah**\n**Status:Cape Detour**", route_uae.replace("Khor Fakkan / ", ""), "📢 **News:** Fujairah discharge with Al Batha border link active\n💰 **Cost:** UAE-Riyadh approx $1,800~$2,200\n🔗 **Bonded:** YES"],
            ["HMM", "Jeddah:🟡(Wait)\nDammam:🔴(Stop)\n**Port:Reviewing**\n**Status:Restricted**", "Suspended", "📢 **Notice:** ME bookings suspended following safety guidelines\n💰 **Cost:** TBD\n🔗 **Bonded:** Consult"]
        ]
...
if st.button(btn_label, type="primary", use_container_width=True):
    ...
    # 7. 심층 전황 및 항만 뉴스 (시간 표시)
    st.markdown("---")
    col_news_1, col_news_2 = st.columns(2)
    with col_news_1:
        st.subheader("🔥 호르무즈 실시간 시황 및 속보" if is_ko else "🔥 Hormuz Crisis Intel")
        war_news = [
            {"s": "Reuters", "t": "1h ago", "txt": "이란 혁명수비대 호르무즈 해협 기뢰 매설 징후로 통항 사실상 마비" if is_ko else "IRGC mine activity signs in Hormuz; transit paralyzed"},
            {"s": "Windward", "t": "3h ago", "txt": "지난 24시간 내 대형 컨테이너선 해협 통과량 '0' 기록" if is_ko else "Zero commercial vessels passed Hormuz in last 24h"}
        ]
        for n in war_news:
    ...
    with col_news_2:
        ...
        port_news = [
            {"p": "Khor Fakkan (UAE)", "t": "2h ago", "txt": "제벨알리 대체 수요 집중으로 터미널 가동률 95% 상회" if is_ko else "Terminal utilization over 95% due to Jebel Ali bypass"}
        ]
        for p in port_news:
    ...
    q_label = "Q. 항구별 사우디 진입 국경 및 리스크" if is_ko else "Q. Border Points and Risks per Port"
    with st.expander(q_label):
        st.write("UAE(코르파칸 등) 이용 시 **Al Batha Border**를 이용하며, 오만(살랄라 등) 이용 시 **Rub Al Khali Border**를 이용합니다." if is_ko else "Use **Al Batha** for UAE ports (Khor Fakkan) and **Rub Al Khali** for Oman ports (Salalah).")
    st.markdown('</div>', unsafe_allow_html=True)

    # 9. 전문가 면책 고지
    disc_title = "Professional Disclaimer"
    disc_txt = f"본 리포트는 {current_time} 기준 최신 외신 및 선사 공식 기보를 기반으로 작성되었습니다. 실제 물류 실행 시에는 **반드시 LX Pantos Saudi Arabia 담당 전문가**를 통해 최종 검증을 받으시기 바랍니다." if is_ko else f"This report is based on current data as of {current_time}. Please consult with **LX Pantos Saudi Arabia experts** for final verification before execution."
    st.markdown(f"""
