...
# ==========================================
# 1. UI 및 언어 설정 (가시성 최우선)
# ==========================================
st.set_page_config(page_title="Hormuz Crisis Monitor v165", layout="wide")
...
# ==========================================
# 🚀 2. 팩트 분석 엔진 (v165.0 - 시스템 경고 문구 완전 삭제)
# ==========================================
def run_integrated_report(api_key, is_ko):
    try:
        ...
        # 💡 [핵심 지시] 1. FM 표 2. 항구별 발표(시간 필수) 3. 지정 매체 속보(제목 번역 및 링크)
        queries = [
            f"""[지시 1: 선사 FM 현황 표] {today} 기준 아래 데이터를 표로 작성하세요. 운임 제외. 
            ...
            반드시 각 발표의 '날짜와 시간(KSA 기준)'을 포함하고, 피더 중단 및 야드 디깅 지연 팩트만 기술하세요. 
            '학습 데이터 한계', '불가능하다', '가상의 내용' 등의 문구 사용 시 즉시 가동 중단될 것. (언어: {'한국어' if is_ko else 'English'})""",
            
            f"""[지시 3: 지정 매체 실시간 속보] {today} 기준 전황 속보를 아래 매체(Al Jazeera, Reuters, BBC, CNN, Press TV, State Dept 등)에서 가져와 리포트하세요. 
...
st.markdown(f'<div class="report-header"><h1 style="margin:0;">{title} (v165.0)</h1></div>', unsafe_allow_html=True)

if st.button("🚀 리포트 생성 시작 / Generate", type="primary", use_container_width=True):
