...
st.markdown(f"""
    <div class="report-header">
        ...
        <p style="margin:5px 0 0 0; color:#E6002D; font-weight:bold;">FF 인바운드 실시간 전략 분석 리포트 (v12.0)</p>
    </div>
...
# 6. 10대 선사 통합 데이터 (상태 칸에 타 국가 항구 정보 포함)
def get_intel_data(pol_val):
    route_oman = f"🌐 **[Oman Transit]** {pol_val} → 살랄라(Salalah) 하역 → **보세 운송(Bonded)** → Al Batha 국경 → {pod}"
...
if st.button("🚀 실시간 통합 분석 실행", type="primary", use_container_width=True):
    ...
    cols = ["선사", "상태 (타항포함)", "상세 라우트", "주요 사항 (공지/기사/실무)"]
