...
# ==========================================
st.set_page_config(page_title="LX Pantos Saudi Intel v137", layout="wide")
...
st.markdown("""
    ...
    .data-row { margin-bottom: 12px; line-height: 1.8; white-space: pre-wrap; font-size: 1.0rem; color: #111; }
...
def run_integrated_report(api_key, is_ko):
    try:
        ...
        # 💡 [핵심 지시] 1. 서론/면피 금지 2. 공식 발표 팩트 3. 선사명 강조 4. 줄바꿈
        queries = [
            f"""당신은 LX 판토스 물류 전문가입니다. {today} 기준, 호르무즈 해협 봉쇄에 따른 10대 선사별 '공식 발표(Official Advisory)'만 리포트하세요. 
            서론, 인사말, 미래 시점 운운하는 면피용 문구는 절대 사용하지 마십시오. 오직 팩트만 전하십시오.
            
            ### <span style='color:#E6002D'>**선사명**</span>
            • **공식 지정 강제 양하기(EOV Port)**: (공식 발표된 실제 하역 항구)
            • **FM 선언 및 부킹 정책**: (공식 발표된 FM 여부 및 부킹 중단/제한 팩트)
            • **화주 추가 부담 비용**: (공지된 Contingency Surcharge, WRS, 재배송 비용 등 금액 팩트)
            • **공식 대체 루트 안내**: (선사가 공식 제안한 육상 우회 경로 팩트)
            
            [대상]: MSC, Maersk, CMA CGM, COSCO, HMM, Hapag-Lloyd, ONE, Evergreen, OOCL, ZIM.
...
# 3. 메인 화면 구성
# ==========================================
st.markdown(f'<div class="report-header"><h1 style="margin:0;">LX PANTOS <span style="font-size:1.1rem; color:#666;">| Saudi Arabia</span></h1><p style="margin:5px 0 0 0; color:#E6002D; font-weight:bold;">호르무즈 위기 통합 관제 리포트 (v137.0)</p></div>', unsafe_allow_html=True)
...
st.markdown(f"""
    <div class="footer">
        ...
        담당: {current_date_str} 기준 실시간 분석 시스템 (v137.0)
    </div>
""", unsafe_allow_html=True)
