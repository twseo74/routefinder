...
# ==========================================
st.set_page_config(page_title="Saudi Intel v146", layout="wide")
...
st.markdown("""
    ...
    .iran-side { border-left: 8px solid #cc0000; background-color: #fff5f5; }
...
# ==========================================
# 🚀 2. 실시간 팩트 분석 엔진 (전황 대조 및 링크 포함)
# ==========================================
def run_integrated_report(api_key):
    try:
        genai.configure(api_key=api_key)
        # 동적 모델 선택으로 404 에러 방지
        ...
        # 💡 [핵심 지시] 1. 선사 FM 팩트 2. 야드 리스크 3. 전황(미국/이란편 구분 및 링크)
        queries = [
            f"""{today} 기준 이란-이스라엘 전쟁에 따른 10대 선사 공식 발표를 리포트하세요. 
            ...
            f"""{today} 기준 이란-이스라엘 전쟁 최신 속보를 [이란/아랍편]과 [미국/서방편]으로 나누어 리포트하세요.
            [작성 양식]:
            1. 성향 구분 (예: 🔴 이란/아랍측 보도 vs 🔵 미국/서방측 보도)
            2. 최신 기사 제목 및 원문 번역
            3. 실제 기사 링크 (URL) 포함 - 클릭 시 바로 이동 가능하게 할 것
            4. 보도 시각 (KSA 기준) 및 물류 영향 분석 포함.
            (언어: 한국어, 줄바꿈 필수)"""
        ]
...
# 🚀 3. 메인 실행부
# ==========================================
st.markdown(f'<div class="report-header"><h1 style="margin:0;">호르무즈 위기 통합 관제 리포트 (v146.0)</h1></div>', unsafe_allow_html=True)

if st.button("🚀 리포트 생성 시작 (실시간 링크 포함)", type="primary", use_container_width=True):
