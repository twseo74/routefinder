...
st.markdown(f"""
    ...
    <div class="update-box"><strong>실시간 시황 및 프로세스 검증 시점:</strong> {current_time}</div>
""", unsafe_allow_html=True)
...
if st.button("🚀 실시간 통합 현황 분석 실행", type="primary", use_container_width=True):
    ...
    with col_news_1:
        ...
        war_news = [
            {"s": "Reuters", "t": "1시간 전", "txt": "이란 혁명수비대 호르무즈 해협 기뢰 매설 징후로 통항 사실상 마비"},
            {"s": "Windward", "t": "3시간 전", "txt": "실시간 데이터: 지난 24시간 내 대형 컨테이너선 해협 통과량 '0' 기록"},
            {"s": "Bloomberg", "t": "오늘 오후", "txt": "사우디 에너지부, 동부 유전 및 주요 정유 시설 경계 태세 강화"},
            {"s": "Lloyd's List", "t": "6시간 전", "txt": "글로벌 물류 보험료 사상 최고치 경신 및 인수 거절 가속화"}
        ]
        for n in war_news:
    ...
    with col_news_2:
        ...
        port_news = [
            {"p": "Khor Fakkan (UAE)", "t": "2시간 전", "txt": "제벨알리 대체 수요 집중으로 터미널 가동률 95% 상회"},
            {"p": "Salalah (Oman)", "t": "오늘 오전", "txt": "오만-사우디 직통 국경행 보세 차량 배차 대기 시간 증가"},
            {"p": "Fujairah (UAE)", "t": "오늘 오전", "txt": "해협 입구 긴장으로 선박 정박 보험 요율 사상 최고치 기록"}
        ]
        for p in port_news:
            st.markdown(f"""<div class="port-info"><span class="time-label">⏱ {p['t']} | {p['p']}</span>{p['txt']}</div>""", unsafe_allow_html=True)

    # 8. 심층 실무 Q&A (매니저님 요청 핵심)
    ...
    with st.expander("Q1. 보세운송(Bonded)과 일반운송(Transloading 후)의 실무적 차이", expanded=True):
        st.write("보세운송은 제3국에서 통관 없이 사우디 리야드 Dry Port까지 실(Seal) 상태로 이동하여 리야드에서 최종 통관하는 방식입니다. 반면 일반운송은 항구 근처에서 화물을 적출하여 사우디 일반 트럭으로 옮겨 실은 뒤 국경에서 통관하는 방식입니다.")
    with st.expander("Q2. 선사 컨테이너 반납지 및 Transloading 필요성"):
        st.write("현재 선사들은 장비 부족으로 컨테이너의 사우디 반출을 엄격히 제한하고 있습니다. 따라서 항구 근처에서 **Transloading(화물 이적)** 후 빈 컨테이너를 해당 항구에 즉시 반납하는 프로세스가 기본입니다.")
    with st.expander("Q3. 항구별 사우디 진입 국경 및 리스크"):
        st.write("UAE(코르파칸 등) 이용 시 **Al Batha Border**를 이용하며, 오만(살랄라 등) 이용 시 **Rub Al Khali Border**를 이용합니다. 각각의 국경 통과 시간과 특수 차량 수급 상황이 다르니 주의가 필요합니다.")
    ...
    st.markdown(f"""
        <div style="background-color: #f8f9fa; border: 1px solid #ced4da; padding: 20px; border-radius: 8px; margin-top: 25px;">
            <p style="color: #495057; font-size: 0.85rem; line-height: 1.6; margin: 0;">
                ...
                본 리포트는 {current_time} 기준 최신 외신 및 선사 공식 기보를 기반으로 작성되었습니다. 
                실제 물류 실행 시에는 <strong>반드시 LX Pantos Saudi Arabia 담당 전문가</strong>를 통해 최종 검증을 받으시기 바랍니다.
            </p>
        </div>
    """, unsafe_allow_html=True)
