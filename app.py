...
def analyze_live_market(api_key, is_ko):
    try:
        ...
        return f"⚠️ AI 분석 오류 발생: {str(e)}\n\n(API 키가 유효하지 않거나, 일시적인 네트워크 문제일 수 있습니다.)"
...
# API 키를 Streamlit 환경 설정에서 몰래 가져옵니다.
try:
...
if st.button(btn_text, type="primary", use_container_width=True):
    if not API_KEY:
        st.error("⚠️ Streamlit Secrets에 'GEMINI_API_KEY'가 설정되지 않았습니다. 관리자에게 문의하세요." if is_ko else "⚠️ 'GEMINI_API_KEY' is not set in Streamlit Secrets.")
    else:
